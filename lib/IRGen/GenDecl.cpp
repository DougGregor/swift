...
llvm::Constant *IRGenModule::emitProtocolConformances() {
  ...
  for (auto *conformance : ProtocolConformances) {
    // Note: emit the protocol conformance now.
    emitProtocolConformance(conformance);

    auto descriptor = getAddrOfProtocolConformanceDescriptor(conformance);
    auto descriptorRef =
      ConstantReference(descriptor, ConstantReference::Direct);

    descriptorArray.addRelativeAddress(descriptorRef);
  }
