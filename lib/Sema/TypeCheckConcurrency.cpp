  ...
  class ActorIsolationChecker : public ASTWalker {
    ...
    bool recordMutableVarParent(MutableVarParent parent, Expr *subExpr) {
      ...
      if (auto inout = dyn_cast<InOutExpr>(subExpr)) {
        if (!recordMutableVarParent(parent, inout->getSubExpr()))
          return false;

        // If the parent is a load, this isn't semantically an inout access.
        // Record the load as the parent of the inout so we can check for
        // this later.
