...
enum class ConstraintKind : char {
  ...
  /// constraint.
  OneWayBind,
  /// The first type will be bound to the second type as a parameter, but only
  /// when the second type has been fully determined (and mapped down to a
  /// concrete type). At that point, this constraint will be treated like a
  /// `Bind` constraint.
  OneWayBindParam,
};
