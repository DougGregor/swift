...
ConstraintSystem::simplifyOneWayConstraint(
  ...
  ConstraintKind bindingKind = ConstraintKind::Bind; // silence warning
  switch (kind) {
  case ConstraintKind::OneWayBind:
  ...
  case ConstraintKind::OneWayBindParam:
    kind = ConstraintKind::BindParam;
    std::swap(first, secondSimplified);
    break;

  default:
    llvm_unreachable("Not a one-way constraint");
  }
  return matchTypes(first, secondSimplified, bindingKind, flags, locator);
}
