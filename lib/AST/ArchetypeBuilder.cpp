...
namespace swift {
  /// Substitute potential archetypes with their archetypes in a particular
  /// generic environment.
  class PotentialArchetypeSubstitution {
    ArchetypeBuilder &builder;
    GenericEnvironment *genericEnvironment;

  public:
    PotentialArchetypeSubstitution(ArchetypeBuilder &builder,
                                   GenericEnvironment *genericEnvironment)
      : builder(builder), genericEnvironment(genericEnvironment) { }

    /// Perform substitution for the given (potentially dependent) type.
    Type subst(Type type);

    /// Perform substitution for the given potential archetype.
    ArchetypeType::NestedType subst(ArchetypeBuilder::PotentialArchetype *pa);
  };
}

Type PotentialArchetypeSubstitution::subst(Type type) {
  // FIXME: Use dyn_cast rather than getAs.
  return type.transform([&](Type type) -> Type {
      if (auto depMemTy = type->getAs<DependentMemberType>()) {
        auto newBase = subst(depMemTy->getBase());
      ...
      if (auto typeParam = type->getAs<GenericTypeParamType>()) {
        ...
        return subst(potentialArchetype).getValue();
      }
...
PotentialArchetypeSubstitution::subst(ArchetypeBuilder::PotentialArchetype *pa){
  ...
  auto archetypeAnchor = pa->getArchetypeAnchor();
  if (archetypeAnchor != pa)
    return subst(archetypeAnchor);

  auto representative = pa->getRepresentative();
  ASTContext &ctx = pa->getRootParam()->getASTContext();
  ...
  if (Type concreteType = representative->getConcreteType()) {
    ...
    if (representative->RecursiveConcreteType ||
      ...
      return NestedType::forConcreteType(
                ErrorType::get(pa->getDependentType(/*allowUnresolved=*/true)));
    }

    SWIFT_DEFER {
      builder.Impl->ConcreteSubs.erase({genericEnvironment, representative});
    };

    return NestedType::forConcreteType(subst(concreteType));
  }
  ...
  if (!representative->RecursiveSuperclassType &&
      ...
      builder.Impl->SuperclassSubs.count({genericEnvironment, representative})
        > 0) {
  ...
  if (auto parent = pa->getParent()) {
    ...
    if (Type concreteParent = parentTy.getAsConcreteType()) {
      ...
      if (auto parentArchetype = concreteParent->getAs<ArchetypeType>()) {
        ...
        return NestedType::forConcreteType(
                           parentArchetype->getNestedTypeValue(pa->getName()));
      }
      ...
      auto type = pa->getDependentType(/*allowUnresolved=*/false);
      if (type->hasError())
      ...
      return NestedType::forConcreteType(subst(memberType));
    }
    ...
    if (auto nested = ParentArchetype->getNestedTypeIfKnown(pa->getName()))
      return *nested;

    // We will build the archetype below.
    assocType = pa->getResolvedAssociatedType();
  } else if (auto type = genericEnvironment->getMappingIfPresent(
                                                      pa->getGenericParam())) {
  ...
  if (ParentArchetype) {
    ...
    ParentArchetype->registerNestedType(pa->getName(),
                                        NestedType::forArchetype(arch));
  } else {
    // Create a top-level archetype.
    arch = ArchetypeType::getNew(ctx, genericEnvironment, pa->getName(), Protos,
                                 Type());

    // Register the archetype with the generic environment.
    genericEnvironment->addMapping(pa->getGenericParam(), arch);
  }
  ...
  if (Type superclass = representative->getSuperclass()) {
    if (superclass->hasTypeParameter()) {
      (void)builder.Impl->SuperclassSubs.insert({genericEnvironment,
        representative});
      SWIFT_DEFER {
        builder.Impl->SuperclassSubs.erase({genericEnvironment,
                                            representative});
      };
      superclass = subst(superclass);
    }
  ...
  if (!representative->getNestedTypes().empty()) {
    ctx.registerLazyArchetype(arch, builder, pa);
...
void ArchetypeType::resolveNestedType(
  ...
  PotentialArchetypeSubstitution substitution(builder, getGenericEnvironment());
  auto potentialArchetype =
    lazyArchetype.second->getNestedType(nested.first, builder);

  auto result = substitution.subst(potentialArchetype);
...
GenericEnvironment *ArchetypeBuilder::getGenericEnvironment(GenericSignature *signature) {
  ...
  PotentialArchetypeSubstitution substitution(*this, genericEnv);
  for (auto pair : Impl->PotentialArchetypes) {
    Type contextType = substitution.subst(pair.second).getValue();
