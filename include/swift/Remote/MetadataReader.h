  ...
  BuiltType readTypeFromMetadata(StoredPointer MetadataAddress,
    ...
    case MetadataKind::Existential: {
      ...
      for (size_t i = 0; i < Exist->Protocols.NumProtocols; ++i) {
        ...
        // FIXME: Egregious hack because of the way we're mangling protocol
        // descriptors.
        if (MangledName.size() > 3 &&
            MangledName.substr(MangledName.size() - 3) == "_pD")
          MangledName = MangledName.substr(0, MangledName.size() - 3) + "P";
